...
int16_t AccelX, AccelY, AccelZ, Temperature, GyroX, GyroY, GyroZ, ECG_PIN;
float temp1, temp2;
int error = 0;
...
void setup() {
  ...
  Wire.begin();
  sensors.begin();
  pinMode(1, INPUT); // Setup for leads off detection LO + ~D5
  pinMode(3, INPUT); // Setup for leads off detection LO - ~D6
...
void loop() {
  ...
  for (byte i = 75 ; i < bufferLength ; i++) {
    ...
    //AD8232
    if ((analogRead(15) == 13513) || (digitalRead(6) == 1)) { // 10 ~ 3v3, 11 ~ GND
      Serial.println('!');
    }
    else {
      // send the value of analog input 0:
      ECG_PIN = analogRead(4);
      Serial.print("ECG: "); Serial.print(ECG_PIN);
    }
